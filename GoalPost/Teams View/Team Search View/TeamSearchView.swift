    ...
    private func searchForTeams() {
        ...
        Task.init {
            ...
            for searchData in QuickCache.helper.teamDictionary.values {
                ...
                } else if let country = currentCountrySearch, let searchCountry = searchData.country, country != "" {
                    if searchCountry.lowercased().contains(country.lowercased()) {
                        searchResults.append(searchData)
                    }
                } else if let team = currentTeamNameSearch, team != "" {
